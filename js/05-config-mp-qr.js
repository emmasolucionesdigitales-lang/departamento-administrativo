...
function abrirConfigMP(){
  document.getElementById("modal-container").innerHTML=
    '<div class="modal-overlay" onclick="cerrarModal(event)"><div class="modal"><div class="modal-drag"></div>'+
      '<h2 style="font-size:1rem;font-weight:600;color:#c0c8d0;margin-bottom:14px;letter-spacing:0.06em;text-transform:uppercase">Config MercadoPago</h2>'+
...
function renderAppUnificada(tipo, lista, filtro, setFiltro, onNuevo, extraTop){
  ...
  var filas=filtrados.map(function(item){
    ...
    return'<tr>'+
      ...
      '<td><code style="color:#c0a060;font-family:monospace;font-weight:700">'+f.pin+'</code></td>'+
      '<td style="font-weight:500;color:#c0c8d0">'+escHtml(f.negocio)+'</td>'+
      '<td style="color:#7080a0;font-size:0.8rem">'+escHtml(f.email)+'</td>'+
      '<td style="color:#7080a0;font-size:0.8rem">'+escHtml(f.celular)+'</td>'+
  ...
  var cards=filtrados.map(function(item){
    ...
    return'<div class="lic-card" style="border-left:2px solid '+cfg.color+'">'+
      ...
      '</div>'+
      '<div class="lic-card-fila"><span class="lbl">PIN</span><code style="color:#c0a060;font-family:monospace;font-weight:700;font-size:0.95rem">'+f.pin+'</code></div>'+
      '<div class="lic-card-fila"><span class="lbl">Negocio</span><span class="val" style="color:#c0c8d0;font-weight:500">'+escHtml(f.negocio)+'</span></div>'+
  ...
  return'<div style="margin:12px 0 10px"><div style="font-size:1.15rem;font-weight:700;color:'+cfg.color+';letter-spacing:0.06em">'+cfg.icon+' '+cfg.label+'</div><div style="font-size:0.68rem;color:#3a4a58;font-family:monospace;margin-top:2px">'+cfg.firebase+'</div></div>'+
    '<div class="grid4" style="margin-bottom:12px">'+
      '<div class="stat-card"><div class="stat-num">'+lista.length+'</div><div class="stat-label">Total</div></div>'+
      '<div class="stat-card"><div class="stat-num" style="color:#4dd9a0">'+activos+'</div><div class="stat-label">Activos</div></div>'+
      '<div class="stat-card"><div class="stat-num" style="color:#d06060">'+inactivos+'</div><div class="stat-label">Inactivos</div></div>'+
    ...
    '<div class="desktop-only card" style="padding:0;overflow:hidden">'+
      '<div class="table-wrap"><table>'+
        ...
        '</tr></thead>'+
        '<tbody>'+(filas||'<tr><td colspan="8" style="text-align:center;color:#3a4a58;padding:30px;font-family:monospace">Sin registros</td></tr>')+'</tbody>'+
      '</table></div>'+
    '</div>'+
    '<div class="mobile-only">'+(cards||'<div style="text-align:center;color:#3a4a58;padding:40px;font-family:monospace">Sin registros</div>')+'</div>'+
    '<p style="font-size:0.7rem;color:#3a4a58;text-align:center;margin-top:8px;padding-bottom:60px;font-family:monospace">'+filtrados.length+'/'+lista.length+' registros · '+cfg.label+'</p>';
}
...
function accionesBotones(tipo,item,f,modo){
  ...
  /* ✏️ Editar */
  if(tipo==="reparto")acc+=`<button class="btn ${s}" title="Editar licencia" style="background:#0a1a2e;color:#4a9eff;border-color:#4a9eff40" onclick="abrirEditarRep('${id}')">✏️</button>`;
  else if(tipo==="kiosco")acc+=`<button class="btn ${s}" title="Editar cliente" style="background:#18102e;color:#a070ff;border-color:#a070ff40" onclick="abrirEditarKio('${id}')">✏️</button>`;
  else if(tipo==="gestion")acc+=`<button class="btn ${s}" title="Editar licencia" style="background:#0a2018;color:#4dd9a0;border-color:#4dd9a040" onclick="abrirEditarGestion('${id}')">✏️</button>`;
  else if(tipo==="polleria")acc+=`<button class="btn ${s}" title="Editar pollería" style="background:#201400;color:#f5a442;border-color:#f5a44240" onclick="abrirEditarPoll('${id}')">✏️</button>`;
  else if(tipo==="reposteria")acc+=`<button class="btn ${s}" title="Editar licencia" style="background:#200818;color:#f5a0c8;border-color:#f5a0c840" onclick="abrirEditarReposteria('${id}')">✏️</button>`;
  else if(tipo==="repartomulti")acc+=`<button class="btn ${s}" title="Editar negocio" style="background:#0a2028;color:#5dffee;border-color:#5dffee40" onclick="abrirEditarMulti('${id}')">✏️</button>`;

  /* 💳 Cobrar */
  acc+=`<button class="btn ${s}" title="Registrar cobro / ver historial de pagos" style="background:#18103a;color:#a070ff;border-color:#a070ff40" onclick="abrirCobrar('${tipo}','${id}')">💳</button>`;

  /* 🔳 Ver QR de acceso */
  acc+=`<button class="btn ${s}" title="Ver código QR de acceso a la app" style="background:#10141a;color:#c0c8d0;border-color:#3a4050" onclick="verQR('${tipo}','${id}')">🔳</button>`;

  /* 📱 Reset dispositivo */
  if(tipo==="reparto")acc+=`<button class="btn ${s}" title="Resetear dispositivo — el cliente podrá activar desde un celular nuevo" style="background:#1a1400;color:#c0a060;border-color:#c0a06040" onclick="resetDispositivo('${id}')">📱</button>`;
  else if(tipo==="gestion")acc+=`<button class="btn ${s}" title="Resetear dispositivo — el cliente podrá activar desde un celular nuevo" style="background:#1a1400;color:#c0a060;border-color:#c0a06040" onclick="resetDispositivoGestion('${id}')">📱</button>`;
  else if(tipo==="kiosco")acc+=`<button class="btn ${s}" title="Resetear dispositivo — el cliente podrá activar desde un celular nuevo" style="background:#1a1400;color:#c0a060;border-color:#c0a06040" onclick="resetDispositivoKio('${id}')">📱</button>`;
  else if(tipo==="polleria")acc+=`<button class="btn ${s}" title="Resetear dispositivo — el cliente podrá activar desde un celular nuevo" style="background:#1a1400;color:#c0a060;border-color:#c0a06040" onclick="resetDispositivoPoll('${id}')">📱</button>`;
  else if(tipo==="reposteria")acc+=`<button class="btn ${s}" title="Resetear dispositivo — el cliente podrá activar desde un celular nuevo" style="background:#1a1400;color:#c0a060;border-color:#c0a06040" onclick="resetDispositivoReposteria('${id}')">📱</button>`;
  else if(tipo==="repartomulti")acc+=`<button class="btn ${s}" title="Resetear dispositivo del dueño — podrá activar desde un celular nuevo" style="background:#1a1400;color:#c0a060;border-color:#c0a06040" onclick="resetDispositivoMulti('${id}')">📱</button>`;
  ...
  if(tipo==="repartomulti")acc+=`<button class="btn ${s}" title="Ver y gestionar repartidores" style="background:#0a2028;color:#5dffee;border-color:#5dffee40" onclick="verRepartidores('${id}')">👥</button>`;
...
function renderReparto(){
  ...
  var extra=pend>0?'<div style="background:#201400;border:1px solid #f5a44250;border-radius:4px;padding:8px 12px;margin-bottom:10px;font-size:0.8rem;color:#f5a442;font-family:monospace">'+pend+' licencia(s) pendiente(s) de activacion</div>':"";
